...
dl_dataset_dyn_url <- function(uuid) {
  switch(
    ...
    "7b7be246-cd65-4f35-b354-faa705cacecc" = {
      d1 <- rvest::read_html('https://novascotia.ca/news/search/?dept=180'); d2 <- rvest::html_elements(d1, 'a'); sub('^\\.\\.', 'https://novascotia.ca/news', rvest::html_attr(d2[grep('New Cases of COVID-19|New Hospital Admissions', rvest::html_text2(d2))][1], 'href'))
    },
